...
public class InformationGroupActivity extends AppCompatActivity {
                ...
                public void onDataChange(@NonNull DataSnapshot snapshot) {
                    ...
                    if (groupp != null){
                        if (group.getUserCreate().equals(mUser.getUid())){
                            ...
                            Toast.makeText(getApplicationContext(), "Bạn không có quyền xóa nhóm", Toast.LENGTH_SHORT).show();
                        }
                    }
                }
                    ...
                    for (DataSnapshot dn: snapshot.getChildren()){
                        ...
                        if (participant.getId().equals(mUser.getUid())){
                            if (participant.getRole() == 1){
                                Toast.makeText(getApplicationContext(), "Bạn là nhóm trưởng không thể rời nhóm", Toast.LENGTH_SHORT).show();
                            }else {
                                ...
                                log.put("log", "rời nhóm "+ group.getGroupName());
    ...
    private void deleteGroupp() {
        ...
        builder.setMessage("Bạn muốn giải tán nhóm");
        ...
        builder.setPositiveButton("Đồng ý", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                log.put("idUser", mUser.getUid());
                if (group.getType() ==1){
                    log.put("log", "xóa nhóm công việc "+ group.getGroupName());
                }else {
                        log.put("log", "xóa nhóm "+ group.getGroupName());
                }
                log.put("time", System.currentTimeMillis());
                DatabaseReference dbReff = FirebaseDatabase.getInstance().getReference("Logs");
                dbReff.push().setValue(log);
